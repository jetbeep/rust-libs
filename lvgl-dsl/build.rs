...
/// Desktop-sim style-prop generation: parse the real `lv_style.h` (from
/// LVGL_INCLUDE_DIRS) and emit `$OUT_DIR/lv_style_props.rs` with the exact
/// `lv_style_prop_t` ids for the LVGL version we link against.
///
...
/// LVGL is repinned. Deriving them from the header keeps the simulator correct
/// for whatever LVGL it actually compiles against.
fn generate_style_props(include_dirs: &str, out_dir: &str) {
    ...
    let src = fs::read_to_string("src/lvgl/static_style.rs").expect("read static_style.rs");
    let master = parse_master_style_props(&src);
    assert!(
        master.iter().any(|(n, _)| n == "LV_STYLE_TEXT_FONT"),
        "failed to parse the style-prop master list from static_style.rs"
    );

    // 2. Locate and parse the real lv_style.h enum.
    let header = find_lv_style_header(include_dirs)
        .expect("could not locate lv_style.h under LVGL_INCLUDE_DIRS");
    println!("cargo:rerun-if-changed={}", header.display());
    let header_src = fs::read_to_string(&header).expect("read lv_style.h");
    let parsed = parse_style_prop_enum(&header_src);
    assert!(
        parsed.contains_key("LV_STYLE_TEXT_FONT"),
    ...
    out.push_str("// @generated by build.rs::generate_style_props from the real lv_style.h\n");
    out.push_str("// the desktop simulator links against. Do not edit by hand.\n");
    ...
    fs::write(Path::new(out_dir).join("lv_style_props.rs"), out)
        .expect("write lv_style_props.rs");
}
